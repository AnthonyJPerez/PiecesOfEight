...
Yii::import('application.extensions.YImage.Image_Driver');

/**
 * Manipulate images using standard methods such as resize, crop, rotate, etc.
 * This class must be re-initialized for every image you wish to manipulate.
 *
 * $Id: YImage.php 3809 2008-12-18 12:48:41Z OscarB $
 *
 * @package    YImage
 * @author     Kohana Team
...
class YImage {
	...
	/**
	 * Creates a new YImage instance and returns it.
	 *
	...
	{
		return new YImage($image, $config);
	}
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(YImage::$allowed_types[$image_info[2]]))
			throw new CException('image type not allowed');

		// YImage has been validated, load it
		$this->image = array
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => YImage::$allowed_types[$image_info[2]],
        ...
        // Load the driver
        Yii::import("application.extensions.YImage.drivers.$driver");
	 ...
	 * wish to use height as master dim, set $image->master_dim = YImage::HEIGHT
	 * This method is chainable.
	 *
	 * @throws  Kohana_Exception
	 * @param   integer  width
	 * @param   integer  height
	 * @param   integer  one of: YImage::NONE, YImage::AUTO, YImage::WIDTH, YImage::HEIGHT
	 * @return  object
	 */
	...
	protected function valid_size($type, & $value)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'master':
				if ($value !== YImage::NONE AND
				    $value !== YImage::AUTO AND
				    $value !== YImage::WIDTH AND
				    $value !== YImage::HEIGHT)
					return FALSE;
			break;
		}
