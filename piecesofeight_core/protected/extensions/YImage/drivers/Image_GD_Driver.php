...
class Image_GD_Driver extends Image_Driver {
    ...
    public function flip($direction)
	{
		...
		if ($direction === YImage::HORIZONTAL)
		{
			for ($x = 0; $x < $width; $x++)
			{
		...
		elseif ($direction === YImage::VERTICAL)
		{
			for ($y = 0; $y < $height; $y++)
			{
	...
	public function resize($properties)
	{
		...
		if ($properties['master'] === YImage::AUTO)
		{
			// Change an automatic master dim to the correct type
			$properties['master'] = (($width / $properties['width']) > ($height / $properties['height'])) ? YImage::WIDTH : YImage::HEIGHT;
		}

		if (empty($properties['height']) OR $properties['master'] === YImage::WIDTH)
		{
			// Recalculate the height based on the width
			$properties['height'] = round($height * $properties['width'] / $width);
		}

		if (empty($properties['width']) OR $properties['master'] === YImage::HEIGHT)
		{
